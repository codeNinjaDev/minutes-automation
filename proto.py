...
def smart_input(prompt, default=""):
...
def askUser(doc, subject, bullets=False):
    subject_section = input("Any " + subject + "s? ")
    if not bullets:
        while subject_section == 'y':
            report(doc, subject=subject)
            subject_section = input("More " + subject + "s? ")
    else:
        if subject_section == 'y':
            with doc.create(Itemize()) as itemize:
                while subject_section == 'y':
                    subject_bullet = input(subject + ": ")
                    itemize.add_item(str(subject_bullet))
                    subject_section = input("More " + subject + "s? ")
                itemize.append(Command("ldots"))

def announcements(doc):
    askUser(doc, "Announcement", bullets=True)
    pass
...
if __name__ == '__main__':
    members = ["Hope Berry", "Shepard Berry", "Peter Chacko", "Thomas Chacko", "Lauren Bettencourt",  "Isabel Bettencourt", "Grace Bettencourt", "Claire Bettencourt", "Sara Bettencourt"]
    ...
    with doc.create(Section('Call to Order')):
        president = smart_input("President: ", default="Shepard Berry")
        prayer = input("Prayer: ")
        ...
        vp = smart_input("Vice President: ", default="")
    ...
    with doc.create(Section('Reports')):
        ...
        with doc.create(Subsection('Officer')):
            ...
            treasurer = smart_input("Treasurer: ", default="")
            balance = input("Treasury Balance: ")
            tr_notes = input("Notes from treasurery report: ")

            hlth_sfty_officer = smart_input("Health and Safety Officer: ", default="")
            hlth_sfty_report = input("Health and Safety Topic: on ")

            cc_delegate = smart_input("County Council Delegate: ", default="")
    ...
    tex = doc.dumps()
    doc.generate_tex("/home/"+datetime.date.today().strftime("%B %d, %Y"))
